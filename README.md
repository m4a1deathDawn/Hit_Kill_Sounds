...
- **音效冷却**：防止快速射击武器导致音效过载
- **DoT 支持**：可选的持续伤害命中音效和击杀音效（流血、燃烧等）
- **近战武器开关**：可选择是否启用近战武器命中音效
...
### v0.8
- **Bug 修复**：修复了击杀敌人后击杀音效可能多次播放的问题
- **新增功能**：新增 DoT 伤害击杀音效开关
- **新增功能**：新增近战武器命中音效开关
- **代码优化**：删除了部分废弃代码，简化了核心逻辑
...
本 MOD 使用 AttackReportManager Hook 来捕获攻击事件：

1. **AttackReportManager Hook**：通过游戏的网络系统捕获攻击事件，同时支持单人和多人模式

MOD 处理攻击的流程如下：
- 通过 Hook 拦截伤害事件
- 判断是命中（普通/弱点）还是击杀（普通/爆头）
- 根据设置检查 DoT 伤害和近战武器过滤
- 根据敌人类型应用目标过滤
- 通过外部音频播放器播放相应的音效
